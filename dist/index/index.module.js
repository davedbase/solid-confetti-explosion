import { createComponent, memo, effect, template } from 'solid-js/web';
import { mergeProps, createSignal, createMemo, onMount, onCleanup, Show, For } from 'solid-js';
import { keyframes, styled } from 'solid-styled-components';

const _tmpl$ = /*#__PURE__*/template(`<div class="particle"><div></div></div>`, 4);
...
const confettiStyles = (node, inOptions) => {
  const opts = inOptions(); // Get x landing point for it

  const landingPoint = mapRange(Math.abs(rotate(opts.degree, 90) - 180), 0, 180, -opts.stageWidth / 2, opts.stageWidth / 2); // Crazy calculations for generating styles
  ...
  const isCircle = opts.particlesShape !== 'rectangles' && (opts.particlesShape === 'circles' || shouldBeCircle(rotationIndex)); // x-axis disturbance, roughly the distance the particle will initially deviate from its target
  ...
  const x4 = round(Math.abs(mapRange(Math.abs(rotate(opts.degree, 90) - 180), 0, 180, -1, 1)), 4); // roughly how fast particle reaches end of its explosion curve

  const y1 = round(Math.random() * BEZIER_MEDIAN, 4); // roughly maps to the distance particle goes before reaching free-fall

  const y2 = round(Math.random() * opts.force * (coinFlip() ? 1 : -1), 4); // roughly how soon the particle transitions from explosion to free-fall
...
const ConfettiExplosion = inProps => {
  ...
  return createComponent(Show, {
    get when() {
      return memo(() => !!isVisible(), true)() && isValid();
    },
        ...
        get children() {
          return createComponent(For, {
            ...
            children: particle => (() => {
              ...
              confettiStyles(_el$, () => ({ ...particle,
                ...props
              }));

              effect(() => _el$2.style.setProperty("--bgcolor", particle.color));
